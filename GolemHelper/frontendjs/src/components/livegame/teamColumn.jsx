// src/components/livegame/teamColumn.jsx
...
export default function TeamColumn({ teamName, participants }) {
    ...
    return (
        ...
        <h2 className="text-lg font-medium text-gray-100">test</h2>
        {participants.map((player) => {
            ...
            <Avatar>
                ...
                <AvatarImage src={player.champion.icon} alt={player.champion.name} />
            </Avatar>
            <div className="flex-1 space-y-1">
                <p className="text-sm font-medium leading-none text-gray-100">{player.name}</p>
                <p className="text-sm text-gray-400">{player.champion.name}</p>
            </div>
